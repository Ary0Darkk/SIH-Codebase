...
# --- 1. Enhanced Feature Extraction Functions (Time and Frequency Domain) ---
...
def extract_features(segment_data, window_size=WINDOW_SIZE, fs=FS):
    ...
    """
    Extract 7 features per window: MAV, ZC, WL, RMS, VAR, MNF, MDF
    Uses 50% overlap.
    """
    features = []
    for i in range(0, len(segment_data) - window_size + 1, window_size // 2):
        ...
        if len(window) == window_size:
            ...
            features.append([mav, zc, wl, rms, var, mnf, mdf])

    return pd.DataFrame(
        features,
        columns=['MAV', 'ZC', 'WL', 'RMS', 'VAR', 'MNF', 'MDF']
    )
...
for gesture, files in GESTURE_FILES.items():
    ...
    for file_name in files:
        ...
        try:
            ...
            # Cleaning: ensure numeric values
...
def plot_raw_signal(data_frame, gesture_name):
    ...
    # Try to take 500 samples for approx 1 second
    start_index = np.random.randint(0, max(len(gesture_df) - 500, 1)) if len(gesture_df) >= 500 else 0
...
ax.set_ylabel('ZC')
ax.set_zlabel('WL')
ax.set_title('3D EMG Features Visualization')
ax.legend()
plt.tight_layout()
...
feature_cols = ['MAV', 'ZC', 'WL', 'RMS', 'VAR', 'MNF', 'MDF']
...
print("\nSaved model to 'gesture_rf_model.pkl' and scaler to 'gesture_scaler.pkl'")


# --- 8. Helper for Real-Time Prediction ---
def predict_gesture(new_signal_1d):
    ...
    - Assumes same sampling rate and preprocessing as training.
    """
    ...
    feat_scaled = scaler.transform(feat_df.values)
    preds = model.predict(feat_scaled)

    # Majority vote over all windows
